...
| `REDSTONE_DATA_SERVICE_ID` | `--redstone-data-service-id` | `redstone-primary-prod` | RedStone data-service id whose signer set the adapters are configured with; packages from any other service are rejected on-chain. |
| `REDSTONE_PUSH` | `--redstone-push` | `true` | RedStone push leg: before a liquidation, push RedStone-signed packages to the RedStone adapter for feeds without a Pyth Pro source (the proxy's RedStone-only feeds and Pyth Core + RedStone feeds) — nothing else keeps those adapters fresh. Packages are verified locally against the adapter's own signer set, threshold and timestamp window (read from the contract) before any gas is spent; an untrusted writer's 40 s minimum interval per feed is enforced inside the contract. Feeds with a Pyth Pro source are never RedStone-pushed (one transaction fewer). Same optional-value form as `DRY_RUN`. |
