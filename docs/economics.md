...
- Size inventory per-asset against the liquidation volume you expect to catch, not against total protocol TVL — a bot with $500 of USDC inventory simply cannot fully liquidate a $50,000 underwater position; it will either skip it (the strategy reports insufficient inventory → `LiquidationOutcome::SkippedUnfunded`, counted by the `liquidations_skipped_unfunded_total` metric) or catch a partial slice of it, repeatedly, across rounds.
