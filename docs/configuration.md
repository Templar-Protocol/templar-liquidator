...
| `LAZER_API_URL` | `--lazer-api-url` | `https://pyth-lazer.dourolabs.app` | Pyth Pro price API for scan-side proxy price composition — the only Pyth Pro scan leg (no on-chain adapter read). Only used when `LAZER_API_TOKEN` is set; without a token, Pyth Pro–sourced and Pyth Core–priced markets are filtered at registration. Must be `https` when the token is set — a bearer token over plain http travels in cleartext, so the bot refuses to start. |
| `LAZER_API_TOKEN` | `--lazer-api-token` | unset | Pyth Pro access token (no anonymous tier). Enables both the scan-side Pyth Pro API leg and the on-chain Pyth Pro push. When unset, Pyth Pro–sourced markets and Pyth Core–priced ones (direct-Pyth and LST oracles, Pyth Core proxy sources — all priced over the symbol bridge below) are filtered at registration (scan prices are off-chain only — there is no adapter read to fall back to). |
| `LAZER_SYMBOLS_URL` | `--lazer-symbols-url` | `https://pyth.dourolabs.app/v1/symbols` | Pyth Pro public symbols endpoint — the Pyth Core → Pyth Pro bridge (each feed carries its Pyth Core price id), which is how direct-Pyth and LST markets and Pyth Core proxy sources are priced from the Pyth Pro API. Fetched at every registry refresh (the previous map is kept on failure; bridge-priced markets are filtered while it is empty). No token rides on it, but it must be `https` — the map decides what gets priced. Only used when `LAZER_API_TOKEN` is set. A feed with no Pyth Pro counterpart still filters its market, with the unmapped Pyth Core id in the reason. |
