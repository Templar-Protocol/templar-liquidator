...
- **Ref Finance** (`swap/ref.rs`) applies a fixed `DEFAULT_MAX_SLIPPAGE_BPS` of 50 (0.5%) — not currently exposed as a CLI/env knob. A swap that would exceed this tolerance fails and is retried per `SWAP_RETRY_*`, or surfaces as a `SwapIssue::Failed` notification if retries are exhausted.
- **1-Click** (`swap/oneclick.rs`) charges a 0.1% fee when `ONECLICK_API_TOKEN` is unset; set the token to avoid it. 1-Click only supports exact-input swaps (spend a known input amount, receive however much the venue returns) — `SwapProvider::quote` is unimplemented for both providers (see [docs/backlog.md](backlog.md)).
