...
**`oracle.rs`** — `OracleFetcher`: scan-side prices are off-chain only. Proxy-oracle feeds are composed from each feed's configured sources in order, taking the first leg that yields a fresh price (the RedStone public API via `redstone.rs`, the token-gated Pyth Pro API via `lazer.rs`, transformer inputs via free view calls); there is no on-chain price read at scan time — an on-chain price is either stale or costs a paid push every scan — so a market whose feeds have no off-chain source is filtered at registration (`offchain_priceable`) rather than failed per scan. Pyth Core–configured feeds — direct-Pyth and LST oracles (transformer applied off-chain over the underlying) and Pyth Core sources inside a proxy — are priced from the Pyth Pro API over the Pyth Core → Pyth Pro symbol bridge (`SharedSymbolMap`); a feed with no Pyth Pro counterpart filters its market with the unmapped id in the reason. Admission returns an `Admitted` verdict (`OracleKind` plus the count of bridge-priced feeds): the bot cannot push to a Pyth Core oracle (Pyth Core updates are Wormhole VAAs), so any bridge-priced feed — every feed of a direct-Pyth/LST market, or a proxy feed sourced from Pyth Core — relies on an external pusher at execution time, which is logged per market at registration. The bridge is fetched with a no-redirect client and a response with no usable feed never replaces a populated map. At execution time it pushes fresh Pyth Pro payloads to each Pyth Pro–sourced adapter (`oracle.updateLazer`) and re-aggregates the proxy immediately before the liquidation transaction, since the market contract reads its own on-chain oracle state — RedStone adapters are relayer-pushed.
...
**`lazer.rs`** — Lazer (Pyth Pro) price API client (`POST /v1/latest_price`, Bearer-token authenticated — Lazer has no anonymous tier), keyed by Lazer feed id, projecting EMA with per-feed staleness and future-skew guards. Scan-side only; used only when `LAZER_API_TOKEN` is set. Also fetches the public symbols endpoint (no token) into the Pyth Core → Pyth Pro bridge, keeping only `stable` feeds with a well-formed Pyth Core id.
