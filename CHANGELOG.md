...
- Off-chain proxy-oracle price composition at scan time. Proxy-backed markets previously required a keeper to have recently pushed the proxy's on-chain price cache before they could even be *scanned* — in a standalone deployment (any fork not running a price-push keeper) every proxy market failed with `Missing price` forever. The scanner now reads each feed's source config from the proxy contract (a free view call) and composes the price off-chain: Pyth sources via Hermes, RedStone sources via the public price API (`REDSTONE_API_URL` / `--redstone-api-url`, default `https://api.redstone.finance`), transformers applied with their on-chain input (also a free view call). Lazer sources compose from the Lazer (Pyth Pro) price API when `LAZER_API_TOKEN` is set (`LAZER_API_URL`, default `https://pyth-lazer.dourolabs.app`; Lazer has no anonymous tier) — fresh prices with no dependence on anyone pushing on-chain state; requests use the widely-carried `fixed_rate@200ms` channel, with per-feed retries across channels when the API rejects a batch (it rejects a whole batch over one feed that lacks the requested channel) — and otherwise via a free view read of the on-chain Lazer adapter contract, which only prices the feed while the adapter's pushes are maintained. Both project the EMA price, the same projection the on-chain proxy consumes, and freshness is gated on each feed's own update timestamp where the API reports one (the response envelope's assembly time is the fallback, taken with a warning since it cannot detect a dead feed). With a token set, `LAZER_API_URL` must be `https` — the bot refuses to start rather than send the bearer token in cleartext. [**breaking**] for forks constructing the crate's types directly: `OracleFetcher::new` and `Liquidator::new` gain an `Option<LazerApiConfig>` parameter (the config type's constructor is what enforces the HTTPS invariant), `ServiceConfig` gains the `lazer_api` field, and `Args` the `lazer_api_url`/`lazer_api_token` fields (CLI/env surfaces are unaffected — both knobs default off). Composition tries a feed's configured sources in order and takes the first fresh price, so a stale or failing leg falls through to the feed's next source before deferring to the on-chain cache read. Scan-time composition costs no gas; execution-time pricing is unchanged — the bot still pushes fresh prices on-chain before a live liquidation and the market contract still enforces its own oracle, freshness bound, and circuit breakers.
