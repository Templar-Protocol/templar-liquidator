...
- *(swap)* Implicit 1-Click deposit accounts are now funded with 0.002 NEAR at creation instead of 1 yoctoNEAR. NEAR bills an account's storage (~182 bytes, ~0.00182 NEAR) to its own balance, so the 1-yocto transfer was rejected and the account never existed — every deposit to a fresh implicit address bounced into a refund after the fact. Creation failures now also stop the swap before the token deposit instead of proceeding toward a guaranteed refund.
...
- *(service)* A missing swap provider no longer aborts the batch-swap pass mid-loop: the check moved out of the per-asset loop, so the remaining assets' bookkeeping and the round summary aren't silently skipped.
