...
- `GET /metrics` — Prometheus text format with `# HELP`/`# TYPE` lines, eight `templar_liquidator_*` series: scan/liquidation counters, a last-successful-scan timestamp gauge, and a per-asset `inventory_reserved_raw{asset=…}` gauge tracking raw units reserved for in-flight liquidations (a value stuck nonzero means a reservation never settled).
