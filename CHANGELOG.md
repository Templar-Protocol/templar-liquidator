...
- *(swap)* A swap whose outcome is unknown is no longer blindly retried. Every 1-Click failure at or after the point where the deposit transaction is submitted — the deposit RPC call not confirming, the deposit-notify call failing, status polling timing out — was previously classified retryable (`Timeout`/`NetworkError`), and the retry wrapper re-ran the *entire* swap: new quote, new deposit address, **second deposit of the same funds**, even though the first deposit may still settle. These now classify as a new `SwapErrorKind::Indeterminate`, which is never auto-retried. Reconciliation is by construction: the next inventory refresh re-reads balances from chain, so a late settlement or refund is reflected before anything sizes a new swap. Truly pre-funds failures (quote request/network errors) remain retryable as before.
