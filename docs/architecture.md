...
**`oracle.rs`** — `OracleFetcher`: scan-side prices are off-chain only. Proxy-oracle feeds are composed from each feed's configured sources in order, taking the first leg that yields a fresh price (the RedStone public API via `redstone.rs`, the token-gated Pyth Pro API via `lazer.rs`, transformer inputs via free view calls); there is no on-chain price read at scan time — an on-chain price is either stale or costs a paid push every scan — so a market whose feeds have no off-chain source is filtered at registration (`offchain_priceable`) rather than failed per scan. Pyth Core is not integrated: Pyth Core no longer supports NEAR (2026-08-26) — `pyth-oracle.near` can never be refreshed again — so a direct-Pyth/LST market is filtered with a reason naming the fix (re-pointing to a proxy oracle) and returns automatically at the refresh after it is re-pointed; Hermes itself is authenticated now, not deprecated. At execution time it pushes fresh Pyth Pro payloads to each Pyth Pro–sourced adapter (`oracle.updateLazer`) and re-aggregates the proxy immediately before the liquidation transaction, since the market contract reads its own on-chain oracle state; for the feeds the Pyth Pro push cannot refresh — those without a Pyth Pro source, and every feed when no Pyth Pro push is available — it pushes RedStone-signed packages to the RedStone adapter (`redstone_push.rs`); nothing else keeps those adapters fresh.
...
**`redstone_push.rs`** — the RedStone push leg's pure parts and gateway client: parses the gateway's signed data packages, serializes them in the RedStone protocol layout, recovers every signature locally (`k256` + keccak) against the adapter's own signer set, threshold and timestamp window (read from the contract via `redstone.getConfig`), and assembles a `write_prices` payload only when the contract will accept it. Target resolution (the feeds the Pyth Pro push cannot refresh) and the write live in `oracle.rs`.
